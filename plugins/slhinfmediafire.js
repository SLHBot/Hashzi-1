...
if (Config.WORKTYPE == 'private') {
	...
	SlHackers.addCMD({ pattern: 'infmediafire ?(.*)', fromMe: true, desc: Lang.DEVICE, dontaddCMDList: false }, async (message, match) => {
	...
	SlHackers.addCMD({ pattern: 'infmediafire ?(.*)', fromMe: false, desc: Lang.DEVICE, dontaddCMDList: false }, async (message, match) => {
		const url = `https://docs-jojo.herokuapp.com/api/mediafire?url=${match[1]}`;
		try {
			const response = await got(url);
