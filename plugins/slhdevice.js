...
if (Config.WORKTYPE == 'private') {
	...
	SlHackers.addCMD({ pattern: 'device ?(.*)', fromMe: true, desc: Lang.DEVICE, dontaddCMDList: false }, async (message, match) => {
	...
	SlHackers.addCMD({ pattern: 'device ?(.*)', fromMe: false, desc: Lang.DEVICE, dontaddCMDList: false }, async (message, match) => {
		const url = `https://docs-jojo.herokuapp.com/api/gsm_arena?q=${match[1]}`;
		try {
			const response = await got(url);
