...
if (Config.WORKTYPE == 'private') {
	...
	SlHackers.addCommand({ pattern: 'device ?(.*)', fromMe: true, desc: Lang.DEVICE, dontaddCommandList: false }, async (message, match) => {
	...
	SlHackers.addCommand({ pattern: 'device ?(.*)', fromMe: false, desc: Lang.DEVICE, dontaddCommandList: false }, async (message, match) => {
		const url = `https://docs-jojo.herokuapp.com/api/gsm_arena?q=${match[1]}`;
		try {
			const response = await got(url);
