...
if (Config.WORKTYPE == 'private') {
    ...
    SlHackers.addCommand({ pattern: 'short ?(.*)', fromMe: true, desc: Lang.URL }, (async (message, match) => {
        ...
        if (match[1] === '') return await message.client.sendMessage(message.jid, SLang.LİNK, MessageType.text, { quoted: message.data });
        ...
        TinyURL.shorten(`${match[1]}`, async (res, err) => {
            ...
            await message.client.sendMessage(message.jid, `*Original Link:* ${match[1]}\n*Short Link:* ` + res, MessageType.text, { quoted: message.data })
        });
    }));
    SlHackers.addCommand({ pattern: 'calc ?(.*)', fromMe: true, desc: Lang.CALC }, (async (message, match) => {
        if (match[1].length < 4) { return await message.client.sendMessage(message.jid, Lang.VALİD, MessageType.text, { quoted: message.data }) }
        if (match[1].includes('+')) {
            var split = match[1].split('+'), sonsayi = split[1], ilksayi = split[0]
            var result = -(-ilksayi - sonsayi)
            try { await message.client.sendMessage(message.jid, Lang.SUC + result, MessageType.text, { quoted: message.data }) }
            catch (err) {
                return await message.client.sendMessage(message.jid, Lang.UNSUC + err, MessageType.text, { quoted: message.data });
            }
        }
        else if (match[1].includes('-')) {
            var split = match[1].split('-'), sonsayicik = split[1], ilksayicik = split[0]
        ...
        else if (match[1].includes('x')) {
            var split = match[1].split('x'), sonsayicarp = split[1], ilksayicarp = split[0]
            ...
            var split = match[1].split('/'), sonsayibol = split[1], ilksayibol = split[0]
