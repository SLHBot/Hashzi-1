...
if (Config.WORKTYPE == 'private') {
    ...
    SlHackers.addCMD({ pattern: 'moddd ?(.*)', fromMe: true, desc: Lang.DEVICE, dontaddCMDList: false }, async (message, match) => {
    ...
    SlHackers.addCMD({ pattern: 'moddd ?(.*)', fromMe: false, desc: Lang.DEVICE, dontaddCMDList: false }, async (message, match) => {
        const url = `https://api.zeks.me/api/happymod?apikey=bimsaraapi&q=${match[1]}`;
        try {
            const response = await got(url);
