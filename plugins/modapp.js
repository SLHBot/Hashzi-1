...
if (Config.WORKTYPE == 'private') {
    ...
    SlHackers.addCommand({ pattern: 'moddd ?(.*)', fromMe: true, desc: Lang.DEVICE, dontAddCommandList: false }, async (message, match) => {
    ...
    SlHackers.addCommand({ pattern: 'moddd ?(.*)', fromMe: false, desc: Lang.DEVICE, dontAddCommandList: false }, async (message, match) => {
        const url = `https://api.zeks.me/api/happymod?apikey=bimsaraapi&q=${match[1]}`;
        try {
            const response = await got(url);
