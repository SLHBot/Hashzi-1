...
if (Config.WORKTYPE == 'private') {
    ...
    SlHackers.addCMD({ pattern: 'sfb ?(.*)', fromMe: true, desc: Lang.DEVICE, dontaddCMDList: false }, async (message, match) => {
    ...
    SlHackers.addCMD({ pattern: 'sfb ?(.*)', fromMe: false, desc: Lang.DEVICE, dontaddCMDList: false }, async (message, match) => {
        const url = `${Config.LOLSITE}api/facebook2?apikey=${Config.LLHAPI}&url=${match[1]}`;
        try {
            const response = await got(url);
