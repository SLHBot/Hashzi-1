...
if (Config.WORKTYPE == 'private') {
    ...
    SlHackers.addCMD({ pattern: 'nvideo ?(.*)', fromMe: true, desc: Lang.T_DESC }, (async (message, match) => {
    ...
    SlHackers.addCMD({ pattern: 'tblend ?(.*)', fromMe: false, desc: Lang.T_DESC }, (async (message, match) => {
    ...
    SlHackers.addCMD({ pattern: 'tblend ?(.*)', fromMe: true, desc: Lang.T_DESC, dontaddCMDList: true }, (async (message, match) => {
        ...
        if (match[1] === 'dodge') {
            ...
            ffmpeg(location)
                ...
                .save('output.mp4')
                .on('end', async () => {
                    await message.sendMessage(fs.readFileSync('output.mp4'), MessageType.video, { caption: '*Produced by :*\n 𝑺̲̅𝑳̲̅𝑯̲̅𝒂̲̅𝒄̲̅𝒌̲̅𝒆̲̅𝒓̲̅𝒔̲̅ 𝑻̲̅𝒆̲̅𝒂̲̅𝒎̲̅ 𝑩̲̅𝒐̲̅𝒕̲̅  - Divide TBlend' });
                });
            return await message.client.deleteMessage(message.jid, { id: downloading.key.id, remoteJid: message.jid, fromMe: true })
        }
        else if (match[1] === 'xor') {
