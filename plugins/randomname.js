...
if (Config.WORKTYPE == 'private') {
  ...
  SlHackers.addCMD({ pattern: 'rdname ?(.*)', fromMe: true, desc: Lang.DEVICE, dontaddCMDList: false }, async (message, match) => {
  ...
  SlHackers.addCMD({ pattern: 'rdname ?(.*)', fromMe: false, desc: Lang.DEVICE, dontaddCMDList: false }, async (message, match) => {
    const url = `https://docs-jojo.herokuapp.com/api/random-name?gender=${match[1]}`;
    try {
      const response = await got(url);
