...
if (Config.WORKTYPE == 'private') {
  ...
  Asena.addCommand({ pattern: 'rdname ?(.*)', fromMe: true, desc: Lang.DEVICE, dontAddCommandList: false }, async (message, match) => {
  ...
  Asena.addCommand({ pattern: 'rdname ?(.*)', fromMe: false, desc: Lang.DEVICE, dontAddCommandList: false }, async (message, match) => {
    const url = `https://docs-jojo.herokuapp.com/api/random-name?gender=${match[1]}`;
    try {
      const response = await got(url);
