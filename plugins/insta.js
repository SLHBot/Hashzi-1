...
Asena.addCommand({ pattern: 'vinsta ?(.*)', fromMe: false, desc: sd }, async (message, match) => {
  ...
  await axios.get(`https://docs-jojo.herokuapp.com/api/insta?url=${userName}`).then(async (response) => {
    ...
    if (resource.url.is_video[0]) {
      await message.sendMessage(Buffer.from(profileBuffer.data), MessageType.video, { caption: 'Made by *🔰𝒮𝐿𝐻𝒶𝒸𝓀𝑒𝓇𝓈 𝒯𝑒𝒶𝓂 𝐵❁𝓉🔰*' })
    }
    else if (!resource.url.is_video[0]) {
      await message.sendMessage(Buffer.from(profileBuffer.data), MessageType.image, { caption: 'Made by *🔰𝒮𝐿𝐻𝒶𝒸𝓀𝑒𝓇𝓈 𝒯𝑒𝒶𝓂 𝐵❁𝓉🔰*' })
    }
  }).catch(async (err) => {
